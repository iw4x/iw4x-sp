...
html_frame::callback_params::callback_params(DISPPARAMS* params, VARIANT* res)
    : result(res) {
...
html_frame::html_frame()
    : in_place_frame_(this), in_place_site_(this), ui_handler_(this),
      client_site_(this), html_dispatch_(this) {
...
doc_host_ui_handler* html_frame::get_ui_handler() { return &this->ui_handler_; }

ole_client_site* html_frame::get_client_site() { return &this->client_site_; }

html_dispatch* html_frame::get_html_dispatch() { return &this->html_dispatch_; }
...
std::shared_ptr<IWebBrowser2> html_frame::get_web_browser() const {
  ...
  if (FAILED(this->browser_object_->QueryInterface(
          IID_IWebBrowser2, reinterpret_cast<void**>(&web_browser))) ||
      !web_browser)
    return {};
...
std::shared_ptr<IDispatch> html_frame::get_dispatch() const {
  ...
  if (FAILED(web_browser->get_Document(&dispatch)) || !dispatch)
    return {};
...
std::shared_ptr<IHTMLDocument2> html_frame::get_document() const {
  ...
  if (FAILED(dispatch->QueryInterface(IID_IHTMLDocument2,
...
void html_frame::create_browser() {
  ...
  class_factory->CreateInstance(nullptr, IID_IOleObject,
                                reinterpret_cast<void**>(&browser_object));
  ...
  this->browser_object_ = std::shared_ptr<IOleObject>(
      browser_object, [](IOleObject* browser_object) {
        if (browser_object) {
...
void html_frame::initialize_browser() {
  ...
  this->browser_object_->DoVerb(OLEIVERB_SHOW, nullptr, this->get_client_site(),
                                -1, this->get_window(), &rect);
...
bool html_frame::set_browser_feature(const std::string& feature, DWORD value) {
  const auto registry_path =
      R"(SOFTWARE\Microsoft\Internet Explorer\Main\FeatureControl\)" + feature;

  HKEY key = nullptr;
  if (RegCreateKeyA(HKEY_CURRENT_USER, registry_path.data(), &key) ==
      ERROR_SUCCESS) {
    RegCloseKey(key);
  }

  key = nullptr;
  if (RegOpenKeyExA(HKEY_CURRENT_USER, registry_path.data(), 0, KEY_ALL_ACCESS,
                    &key) != ERROR_SUCCESS) {
    return false; // Error :(
  }
  ...
  DWORD type{};
  auto is_new = true;
  if (RegQueryValueExA(key, name.data(), nullptr, &type, nullptr, nullptr) ==
      ERROR_SUCCESS) {
    is_new = false;
  }

  RegSetValueExA(key, name.data(), 0, REG_DWORD,
...
bool html_frame::load_html(const std::string& html) const {
  ...
  if (FAILED(SafeArrayAccessData(safe_array,
...
void html_frame::register_callback(
    ...
    const std::function<void(callback_params*)>& callback) {
