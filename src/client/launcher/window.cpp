...
void window::create(const std::string& title, const int width, const int height,
  ...
  this->handle_ = CreateWindowExA(NULL, this->wc_.lpszClassName, title.data(),
                                  flags, x, y, width, height, nullptr, nullptr,
                                  this->wc_.hInstance, this);
...
void window::set_callback(
    const std::function<LRESULT(window*, UINT, WPARAM, LPARAM)>& callback) {
  ...
  if (message == WM_DPICHANGED) {
    ...
    if (get_dpi) {
      ...
      if (dpi != this->last_dpi_) {
        ...
        MoveWindow(*this, rect.left, rect.top, int(width * scale),
