...
std::string get_temp_filename() {
  ...
  GetTempFileNameA(pathname, "iw4sp-", 0, filename);
...
HANDLE write_dump_to_temp_file(const LPEXCEPTION_POINTERS exceptioninfo) {
  ...
  if (!MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(),
    ...
    MessageBoxA(
        nullptr,
        "There was an error creating the minidump! Hit OK to close the program",
...
std::string create_minidump(LPEXCEPTION_POINTERS exceptioninfo) {
  ...
  const auto _ = gsl::finally([file_handle]() { CloseHandle(file_handle); });
